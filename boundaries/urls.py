...
urlpatterns = patterns('',
    ...
    url(r'^boundaries/(?P<geo_field>shape|simple_shape|centroid|extent)$', BoundaryListView.as_view(),
        name='boundaries_boundary_list'),
    url(r'^boundaries/(?P<set_slug>[\w_-]+)/$', BoundaryListView.as_view(), name='boundaries_boundary_list'),
    url(r'^boundaries/(?P<set_slug>[\w_-]+)/(?P<geo_field>shape|simple_shape|centroid|extent)$', BoundaryListView.as_view(),
        name='boundaries_boundary_list'),
    url(r'^boundaries/(?P<set_slug>[\w_-]+)/(?P<slug>[\w_-]+)/$', BoundaryDetailView.as_view(),
        name='boundaries_boundary_detail'),
    url(r'^boundaries/(?P<set_slug>[\w_-]+)/(?P<slug>[\w_-]+)/(?P<geo_field>shape|simple_shape|centroid|extent)$',
